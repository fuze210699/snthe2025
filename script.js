...
const letterData = [
    ...
    },
    {
        ...
        date: "25/10/2025",
        title: "Đêm Halloween",
        preview: "Một đêm ma thuật đặc biệt...",
        content: `
            <h1>🎃 Halloween Vui Vẻ</h1>
            <p>Boo! Tui có làm bạn giật tui không? 👻</p>
            <p>Halloween năm nay, tui muốn mời bạn cùng đi xem phim kinh dị. Đừng lo, tui sẽ bảo vệ bạn nếu có ma nào xuất hiện! (Dù tui cũng sợ lắm 😅)</p>
            <p>Trick or treat? Tui chọn treat - vì được gặp bạn đã là món quà ngọt ngào nhất rồi! 🍭</p>
            <p>P.S: Tui đã chuẩn bị sẵn kẹo rồi đó!</p>
        `,
        isUnlocked: false
    },
    {
...
function startCelebration() {
    ...
    if (welcomeScreen && animationScreen) {
        ...
        animationScreen.style.transition = 'all 0.6s cubic-bezier(0.25, 0.46, 0.45, 0.94)';

        // Reset all animations before starting
        resetAnimationElements();

        // Fade out welcome screen
        welcomeScreen.style.transition = 'all 0.6s cubic-bezier(0.25, 0.46, 0.45, 0.94)';
...
function triggerBirthdayText() {
    ...
    if (subtitleElement) {
        subtitleElement.style.animation = 'none';
        setTimeout(() => {
            subtitleElement.style.animation = 'rainbow-fade-in 1.5s ease-out 0.5s both';
        }, 100);
    }
}
...
function createFireworks() {
    const colors = ['#ff6b9d', '#4ecdc4', '#45b7d1', '#96c93f', '#ffd700', '#ff4757', '#3742fa'];
...
function createHeartDrop(hearts) {
    ...
    heart.style.animation = `heart-fall ${2 + Math.random() * 2}s linear forwards`;
...
function createMegaConfetti() {
    const colors = ['#ff6b9d', '#4ecdc4', '#45b7d1', '#96c93f', '#ffd700', '#ff4757', '#3742fa', '#ff9ff3', '#54a0ff'];
...
function createEnhancedConfettiPiece(colors) {
    ...
    confetti.style.animation = `enhanced-confetti-fall ${2 + Math.random() * 2}s linear forwards`;
...
function showMenu() {
    ...
    if (animationScreen && menuScreen) {
        ...
        setTimeout(() => {
            ...
            menuScreen.style.transition = 'all 0.6s cubic-bezier(0.25, 0.46, 0.45, 0.94)';
